...
s=3.5  # step size for MH step for sampling a
...
ratio = rep(0, Nsim) # acceptance probabilities in the MH step for IBP concentration prameter
val1=rep(0, Nsim) 
...
alphath = 3; a2 = 2; b2 = 1 #100/Rave # hyperparameters of the slab distribution
betath = rep(0, Nsim)
betath[1] <- a2/b2
### hyperparameter of the spike
alpha_inf = 21; as = 1; bs = 1 #100/Rave # hyperparameters of the spike so that mean=0.05
beta_inf = rep(0, Nsim)
beta_inf[1] <- as/bs
...
for (g in 2:Nsim) { # open Gibbs sampler
  ...
  # sum of variances
    ...
    # Step 12 - Metropolis-Hastings step to sample a_stick (shape parameter of the slab probability tau)
...
# Block 3 (b)
# MH step to sample a - Dirichlet parameter for hte mixture weights
...
plot(a, type="l")
print(paste("mean value of a", mean(a[Burnin:Nsim])))
print(paste("acceptance rate of a", sum(val)/Nsim))
# checking IBP parameter a_stick
plot(a_stick, type="l")
mean(a_stick[Burnin:Nsim])
print(paste("mean value of a", mean(a_stick[Burnin:Nsim])))
print(paste("acceptance rate of a", sum(val1)/Nsim))
